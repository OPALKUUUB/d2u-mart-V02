...
function Basket() {
  const [isShowPopupBasket, setIsShowPopupBasket] = useRecoilState(
    isShowPopupBasketState
  );
  ...
  useEffect(() => {
    ...
    itemInBasket.forEach((item) => {
      price +=
        Number(item.price.replace("￥", "").replaceAll(",", "")) * item.count;
      count += item.count;
    });
  ...
  return (
    <>
      <div
        className={`fixed bottom-2 sm:bottom-7 left-10 ${
          isShowPopupBasket ? "bg-[#c8ceca]" : "bg-[#ffae00]"
        } rounded-full p-3 flex justify-center items-center cursor-pointer hover:scale-105 ease-in duration-200 z-[999]`}
        style={{
      ...
      >
        {countItemInBasket !== 0 && (
          <div
            className={`absolute top-0 right-0 w-5 h-5 text-[14px] text-white flex justify-center items-center rounded-full shadow-lg bg-[#09092d]`}
          >
            {countItemInBasket}
          </div>
        )}
      ...
      <AnimatePresence>
        {isShowPopupBasket && (
          <motion.div
            className="fixed bottom-[80px] left-7 sm:bottom-7 sm:left-[120px] flex flex-col items-center w-screen max-w-[340px] sm:max-w-[380px] max-h-[550px] h-screen rounded-xl shadow-lg bg-[#fff4ed] z-[999] gap-3 overflow-x-clip"
          ...
          >
            ...
            <div
              className="w-full h-full max-h-[340px] overflow-y-auto mt-[50px] flex flex-col items-center divide-y-[1px] divide-gray-300"
              onWheel={(e) => {
                e.stopPropagation();
                e.preventDefault();
              }}
            >
              {itemInBasket.map((item, index) => (
                <div
                  key={`itemInBasket${index}`}
                  className="w-full bg-white px-[10px] py-3 flex gap-[10px] items-center relative"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-5 w-5 cursor-pointer absolute top-4 right-2 text-gray-400"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2}
                    onClick={() => {
                      setItemInBasket(
                        itemInBasket.filter(
                          (keepItemInBasket) => keepItemInBasket.id !== item.id
                        )
                      );
                    }}
                  >
                  ...
                  <div className="flex w-full justify-between items-center">
                    ...
                    <div className="h-full flex items-center">
                      ...
                      <p className="m-0 text-[20px]">
                        {calculateEachItemPrice(item.price, item.count)}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex w-full justify-between items-center px-3">
              <p className="m-0 text-[14px] sm:text-[16px]">
                จำนวนสินค้า {countItemInBasket} ชิ้น
              </p>
            ...
            <img
              ...
              alt=""
              className=" absolute -bottom-5 -left-4 w-[220px]"
            />
          </motion.div>
        )}
      </AnimatePresence>
