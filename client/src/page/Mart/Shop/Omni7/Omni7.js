          ...
          <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 justify-items-center gap-x-2 gap-y-2">
            {allItemData.map((item,index)=>{
              if(index >= currentPage * 18 && index < (currentPage+1)*18){
                return <SubCard key={`subcard${index}`} text = {item?.name} image = {item?.image} price={item?.price} item={item} />
              }
            })}
          </div>
          </div>
              ...
              >
                {pageIndexList?.map((page , index)=>(
                  <SwiperSlide key={`pageIndexList${index}`}>
                    <div className={`cursor-pointer p-2 rounded-xl text-gray-500 border-gray-300 border-[2px] flex items-center justify-center hover:scale-110 ease-linear duration-150 ${currentPage === index && 'bg-[#c8ceca]'}`}
                      onClick={()=>{
